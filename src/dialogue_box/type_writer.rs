use super::BoxContext;
use bevy::{
    input::{keyboard::KeyboardInput, ButtonState},
    prelude::*,
    text::{ComputedTextBlock, TextLayoutInfo},
};
use bevy_bits::text::{TypeWriterCommand, TypeWriterSection};
use bevy_sequence::prelude::{FragmentId, IntoFragment};

#[derive(Component, Debug, Default, Clone)]
pub struct TypeWriterFont {
    pub handle: Handle<bevy::text::Font>,
    pub size: f32,
    pub color: Color,
}
...
#[derive(Component)]
pub struct TypeWriterTimer(pub Timer);

pub fn update_section_with_index(
    ...
    for (entity, section, TypeWriterIndex(index), info) in type_writers.iter() {
        let end = *index.min(&section.text.value.len());
        if info.is_none_or(|i| i.glyphs.len() != end) {
            println!("updating text based on index");
            commands
                .entity(entity)
                .insert(Text2d::new(section.text.value[..end].to_owned()));
        }
    }
}
...
pub fn scroll_text(
    ...
    // child_spans: Query<Entity, With<TextSpan>>,
    time: Res<Time>,
    ...
    for (section, mut index, mut timer, mut text, children) in writers.iter_mut() {
        timer.0.tick(time.delta());

        if timer.0.just_finished() {
            let mut increment = true;

            for ic in section.commands.iter() {
                if ic.index == index.0 {
                    match ic.command {
                        TypeWriterCommand::Clear => {
                            ...
                            //         if let Ok(child) = child_spans.get(*child) {
